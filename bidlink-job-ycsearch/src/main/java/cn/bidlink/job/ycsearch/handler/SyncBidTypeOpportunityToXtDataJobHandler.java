...
//                                  + "    project.REGION AS areaStr,\n"
                ...
                + "         pip.TENDER_NAMES,\n"
                + "         nb.CREATE_TIME,\n"
                + "         if(nb.BID_ENDTIME,nb.BID_ENDTIME,nb.TECHNICAL_ADVICE_CUT_TIME) as BID_ENDTIME\n"
                + "      FROM\n"
                + "         proj_inter_project pip\n"
                + "      JOIN notice_bid nb ON pip.id = nb.PROJECT_ID\n"
                + "      WHERE\n"
...
//                                    + "    project.REGION AS areaStr,\n"
                ...
                + "         pip.TENDER_NAMES,\n"
                + "         pp.CREATE_TIME,\n"
                + "         if(nb.BID_ENDTIME,nb.BID_ENDTIME,pp.END_TIME) as END_TIME\n"
                + "      FROM\n"
                + "proj_inter_project pip\n"
                + "JOIN pqt_prequalify pp ON pip.id = pp.PROJECT_ID AND pip.COMPANY_ID = pp.COMPANY_ID\n"
                + "LEFT JOIN notice_bid nb ON nb.PROJECT_ID = pip.id\n"
                + "WHERE\n"
                + "   pip.TENDER_MODE = 1\n"
                + "AND pip.IS_PREQUALIFY = 1\n"
                + "AND pip.IS_TWO_STAGE = 2\n"
